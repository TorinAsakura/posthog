...
`hogli start -y -d`, and `setup_dev --no-data`, then reports `starting`, `ready`, or `failed` in
`/tmp/posthog-preview/status.json`, which the wait activity polls. Startup peaks around 19 GB, so
