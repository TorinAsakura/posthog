...
/**
 * The level and service selections, as the dedicated query fields.
 *
 * Three lookups behind the viewer read level and service from these fields alone: the facet value
...
export function columnQueryFieldsOf(filterGroup: UniversalFiltersGroup): ColumnQueryFields {
    return {
        severityLevels: facetSelection(filterGroup, SEVERITY_LEVEL_FILTER).included as LogsQuery['severityLevels'],
        serviceNames: facetSelection(filterGroup, SERVICE_NAME_FILTER).included,
    }
}
...
export interface logsViewerFiltersLogicValues {
    columnQueryFields: ColumnQueryFields
    dateRange: DateRange
    filterGroup: UniversalFiltersGroup
...
export interface logsViewerFiltersLogicMeta {
    ...
    __keaTypeGenInternalSelectorTypes: {
        ...
        columnQueryFields: (filterGroup: UniversalFiltersGroup) => ColumnQueryFields
        queryFilterGroup: (
            filterGroup: UniversalFiltersGroup,
            pinnedFilters: UniversalFiltersGroup | undefined
        ) => UniversalFiltersGroup
...
export const logsViewerFiltersLogic = kea<logsViewerFiltersLogicType>([
    ...
    selectors({
        ...
        // can't pick them up.
        // Scope for the three lookups that still read the dedicated fields (see columnQueryFieldsOf).
        columnQueryFields: [
            (s) => [s.filterGroup],
            (filterGroup: UniversalFiltersGroup) => columnQueryFieldsOf(filterGroup),
        ],
        queryFilterGroup: [
