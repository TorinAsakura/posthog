...
# The launcher runs with a scrubbed environment: the sandbox env carries GITHUB_TOKEN and
# POSTHOG_PERSONAL_API_KEY, and everything the dev stack spawns (Django, the plugin server,
# every compose container) would inherit them. The stack exports what it needs itself, so
# the only inheritance left is this fixed PATH, which resolves image binaries and nothing
# from the agent-writable workspace. /usr/local/go/bin is listed even though the bake
# symlinks go into /usr/local/bin; node, pnpm and docker come from /usr/bin, uv and phrocs
# from /usr/local/bin, and hogli from the repo venv the script activates.
...
def _already_previewing(run_id: str, sandbox_id: str) -> bool:
    """True when this run already recorded a ready preview on this very sandbox.

    Keeps a resumed execution (continue-as-new, workflow retry) from relaunching a stack
    that is already serving. The script's own flock covers the races this misses.
    """
...
def _launch_preview(input: StartDevStackPreviewInput) -> StartDevStackPreviewOutput:
    ...
    manifest = sandbox.execute(f"test -f {shlex.quote(BAKE_MANIFEST_PATH)}", timeout_seconds=30)
    ...
    # Backgrounding the launcher makes its own exit status meaningless, so the state
    # directory and the script land in checked steps of their own first.
    prepared = sandbox.execute(f"mkdir -p {shlex.quote(STATE_DIR)}", timeout_seconds=30)
    ...
    sandbox.write_file(SCRIPT_SANDBOX_PATH, script.encode())
...
def _read_log_tail(sandbox: SandboxBase) -> str:
    try:
        ...
        return ""
    # The launcher echoes the commands it runs, and a run's log can pick up an env
    # assignment along the way, so the tail goes through the same scrubber sandbox
    # commands do before it reaches a user-visible log line.
...
async def start_dev_stack_preview(input: StartDevStackPreviewInput) -> StartDevStackPreviewOutput:
    ...
    with log_activity_execution("start_dev_stack_preview", sandbox_id=input.sandbox_id, **ctx.to_log_context()):
        ...
        try:
            ...
            return StartDevStackPreviewOutput(started=False, reason=f"error:{type(error).__name__}")

        if not output.started:
            logger.info("dev_stack_preview_not_started", run_id=ctx.run_id, reason=output.reason)
            return output

        # The start activity runs with maximum_attempts=1, so one launch counts once.
        DEV_STACK_PREVIEW_TOTAL.labels(outcome="started").inc()
...
async def wait_dev_stack_preview(input: WaitDevStackPreviewInput) -> WaitDevStackPreviewOutput:
    ...
    with log_activity_execution("wait_dev_stack_preview", sandbox_id=input.sandbox_id, **ctx.to_log_context()):
        ...
        await _emit_preview_warning(
            ctx.run_id,
            f"The PostHog preview did not start ({detail}). The task continues without it."
            + (f"\n{tail}" if tail else ""),
        )
