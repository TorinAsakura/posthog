...
def _sandbox(mocker, *, manifest_exit_code=0, prepare_exit_code=0, launch_exit_code=0, status_payload=None):
    sandbox = MagicMock()
...
@pytest.mark.parametrize(
    ...
    [
        ({"side_effect": RuntimeError("sandbox gone")}, "error:RuntimeError"),
        ({"prepare_exit_code": 5}, "prepare_exit_5"),
        ({"launch_exit_code": 7}, "launch_exit_7"),
    ],
)
def test_start_reports_failures_instead_of_raising(
    task_context, activity_environment, mocker, progress, failure, expected_reason
):
    if "side_effect" in failure:
        mocker.patch(f"{MODULE}.Sandbox.get_by_id", side_effect=failure["side_effect"])
    else:
        _sandbox(mocker, **failure)
