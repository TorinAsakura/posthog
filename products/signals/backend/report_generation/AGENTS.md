...
Beside `title`/`summary`, the presentation step authors three structured one-liners the inbox renders without parsing the prose: `headline` (the summary's tl;dr as its own field), `impact` (one quantified sentence on who is affected and how much), and `recommended_action` (the reader's next step; `None` when the report isn't actionable). Five record fields extend the same idea — the report as a record, prose as its rendering: `cause` (the mechanism, one sentence), `cause_location` (file:line / component / surface; renders as "Where"), `fix_size` (effort shape, never guessed), `not_this` (the adjacent thing the report deliberately is not — the reader's "isn't this already handled?" as a field), and `confidence` (the claim ledger: `{verified, measured, inferred, unverified}` lists, so "read the code myself" and "couldn't check" stop sharing one confident voice; an empty `verified` list marks a speculative report at a glance). All of these live as nullable columns on `SignalReport` (the ledger as JSON), written and replaced together with `title`/`summary` on every `IN_PROGRESS -> READY`/`PENDING_INPUT` transition — a rewrite that omits one withdraws it, since the fields describe the current summary, and a `None` means the agent didn't have it, never to be papered over with prose. Reports written before a field existed carry `None` and readers fall back to deriving what they can from the summary. The scout `emit_report`/`edit_report` channel does not author them yet.
